...
ifneq ($(VERBOSE), true)
	...
	echo_string = [$(TARGET_ARCH) ($(BUILD_MODE))]
	ifeq ($(COLOR_TTY),true)
		echo_prog := $(shell if echo -e | grep -q -- -e; then echo echo; else echo echo -e; fi)
		echo_cmd = @$(echo_prog) "$(COLOR)$(echo_string) $(call strip_top,$(1))$(NOCOLOR)";
	else
		echo_cmd = @echo "$(echo_string) $(call strip_top,$(1))";
	endif
else # Verbose output
	V =
	echo_cmd =
...
COMPILE.S = $(call echo_cmd,AS $<) $(CC) $(ASFLAGS) $(CFLAGS) $(TARGET_ARCH) -c 
COMPILE.c = $(call echo_cmd,CC $<) $(CC) $(CFLAGS) $(TARGET_ARCH) -c 
COMPILE.cc = $(call echo_cmd,CXX $<) $(CXX) $(CXXFLAGS) $(TARGET_ARCH) -c 
COMPILE.cpp = $(call echo_cmd,CXX $<) $(CXX) $(CXXFLAGS) $(TARGET_ARCH) -c 

LINK.c = $(call echo_cmd,LINK $@) $(CC) $(CFLAGS) $(LDFLAGS) $(TARGET_ARCH)
LINK.cc = $(call echo_cmd,LINK $@) $(CXX) $(CXXFLAGS) $(LDFLAGS) $(TARGET_ARCH)
LINK.cpp = $(call echo_cmd,LINK $@) $(CXX) $(CXXFLAGS) $(LDFLAGS) $(TARGET_ARCH)
...
$(eval define skeleton \
				$(foreach rule, $(AUTO_RULES),${\n} \
					$$(OBJPATH)/%$(subst :,: $$(1)/%,$(rule)) | \
					$$(OBJPATH); $$(value MKDIR_RULE) && $$(value COMPILECMD))${\n} \
        $(foreach rule,$(AUTO_TD_RULES),${\n} \
					$$(OBJPATH)/%$(subst :,: $$(1)/%,$(subst &, $$(OBJPATH)/%,$(rule))) | \
					$$(OBJPATH); $$(value COMPILECMD_TD))${\n} \
				endef)
