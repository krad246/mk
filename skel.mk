...
VERB_VARS := UT_LIBS BIN_LIBS AUTO_BIN_SRC NO_AUTO_BIN
...
DIR_LDFLAGS 	= $(LDFLAGS_$(@RD))
DIR_LDLIBS 		= $(LDLIBS_$(@RD))

########################################################################
#                       Global flags/settings                          #
########################################################################

# Detect information about the host
HOST_OS			:= $(shell uname -s 2>/dev/null | tr "[:upper:]" "[:lower:]")
HOST_ARCH		:= $(shell uname -m)

# Basic settings for all compilers
WARNINGS				= -Wall -Wextra -Werror -Wunused
BINFLAGS				= -ffreestanding -nostartfiles -nostdlib

COPTS 					= -std=c11 $(WARNINGS) $(BINFLAGS)
CXXOPTS					= -std=c++17 -fno-exceptions $(WARNINGS) $(BINFLAGS)
...
# precedence.
CDEPS						:= -MMD $(DIR_INCLUDES) $(addprefix -I,$(INCLUDES))
CFLAGS 					=	$(CFLAGS_$(@)) $(COPTS) $(CDEPS)
CXXFLAGS 				= $(CXXFLAGS_$(@)) $(CXXOPTS) $(CDEPS)
CPPFLAGS				:= $(CXXFLAGS)
...
LDFLAGS = $(LDFLAGS_$(@)) $(DIR_LDFLAGS) $(addprefix -L,$(LIBDIRS_$(@RD))) $(BINFLAGS)
...
# List of C/C++ headers generated at build-time. The build-system
# enforces header generation in the 1st pass and building C/C++
# in the second pass.
GENERATED_HEADERS =
...
# Append DIR_ flags after config rules have been loaded so that we can
# override global flags at the directory layer.
CFLAGS 		+= $(DIR_CFLAGS)
